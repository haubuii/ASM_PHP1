...
if (isset($_POST['search'])) {
    ...
    if (isset($_GET['trang'])) {
        $trang_so = $_GET['trang'];
        $start = ($trang_so - 1) * 5;
        $sql = "SELECT * FROM product inner join category on product.category_id = category.category_id ORDER BY product_name DESC LIMIT $start, 5";
        $query = mysqli_query($connect, $sql);
    } else {
        // $sql = "SELECT * FROM product inner join category on product.category_id = category.category_id ORDER BY product_name DESC LIMIT 0, 5";
        $sql = "SELECT * FROM product inner join category on product.category_id = category.category_id ORDER BY product_name DESC LIMIT 0, 5";
        $query = mysqli_query($connect, $sql);
    }
}
$row_count = mysqli_num_rows(mysqli_query($connect, "SELECT * FROM product inner join category on product.category_id = category.category_id"));
            ...
            <tbody>
                ...
                while ($row = mysqli_fetch_array($query)) { ?>
                    <tr>
                        ...
                        <td>
                            <img src="<?php echo $row['product_image'] ?>" alt="Ảnh">
                        </td>
                        ...
                        <td scope="col">
                            <a href="index.php?page_layout=xoa&id=<?php echo $row['product_id']; ?>">Xoá</a>
                        </td>
                    </tr>
                <?php } ?>
